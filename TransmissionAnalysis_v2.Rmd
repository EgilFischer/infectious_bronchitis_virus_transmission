...
```{r , echo=FALSE}
plotdata <- sir.data[[1]]
plotdata$check <- plotdata$S +plotdata$I+plotdata$R -plotdata$N
...
<!-- # Regression estimation of the transmission coefficient -->
<!-- Using a complementary log-log link function in a generalized linear model, the transmission coefficient (number of infected birds in one day by one infectious bird) is estimated. -->

<!-- ```{r include = FALSE} -->
<!-- sir.data[[1]]$deltat <- 1 -->
<!-- fit<-glm(cbind(C, S- C)~Vaccinated ,offset= log(deltat*I/N),family = binomial(link = "cloglog"), data = sir.data[[1]]%>%filter(Strain == "DMV" & I >0)) -->

<!-- summary(fit) -->

<!-- exp(cumsum(fit$coefficients)) -->
<!-- ``` -->

<!-- ```{r tablebeta} -->
<!-- kable(cbind(summary(fit)$coefficients, confint(fit)), -->
<!--       caption = c("Estimate of log(beta) using glm."), -->
<!--       digits = 3) -->

<!-- kable(rbind(c("Estimate","2.5%","97.5%"),signif(cbind(exp(cumsum(summary(fit)$coefficients[,1])), exp(cumsum(confint(fit)[,1])),exp(cumsum(confint(fit)[,2]))),digits =3)), -->
<!--       caption = c("Transform to beta"), -->
<!--       digits = 3) -->

<!-- ``` -->
<!-- # Regression estimation of the transmission coefficient - linear regression -->
<!-- ```{r} -->
<!-- sir.data[[1]]<- sir.data[[1]]%>%mutate(X = S * I/N) -->
<!-- sir.data[[1]]%>%filter(Strain == "DMV" & X >0 )%>%group_by(Vaccinated)%>%summarise(beta = mean(C/X), -->
<!--                           beta_sd = sd(C/X), -->
<!--                           ) -->
<!-- lmfit<-lm(C ~  X : Vaccinated ,  data = sir.data[[1]]%>%filter(Strain == "DMV" )) -->
<!-- summary(lmfit) -->
<!-- ``` -->
