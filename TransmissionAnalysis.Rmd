...
* Estimation of $\beta$ (transmission coefficient in birds per day) in vaccinated and unvaccinated birds using generalized linear model
...
```{r include = FALSE}
sir.data[[1]]$deltat <- 1
fit<-glm(cbind(C, S- C)~Vaccinated ,offset= log(deltat*I/N),family = binomial(link = "cloglog"), data = sir.data[[1]]%>%filter(Strain == "DMV"))
