...
source("./src/mapIBVdataCopyNumber.R")
...
colnames(res)<- c("Vaccinated","Estimate","95%-LL", "95%-UL","pval.above1","LogLik")

kable(cbind(res),
      caption = c("Estimate of $R$ based on the final size. Estimate = best value, 95%-LL= lower limit,95%-UL = upper limit, pval.above1 = probability $R$ is above 1, LogLik is log-likelihood"),
      digits = 2)
```
...
fit.beta <- glm(cbind(C,S-C)~Group,offset =-log(I/N),family = binomial(link = "cloglog"),data = glmdat[glmdat$Vaccinated == "No",])
drop1(fit.beta)
...
```{r}
inf.per.data$replication<- str_sub(inf.per.data$Group,-1)
ggplot(inf.per.data)+
  geom_histogram(aes(infT, fill = replication),binwidth = 1, position = "stack")+
  scale_fill_grey() +
  facet_grid(Challenge~Vaccinated, labeller = labeller(
    Vaccinated = c(Yes = "Vaccinated", No = "Unvaccinated"),
    Challenge = c(direct = "Direct", Contact = "Contact")
  ))+theme_bw()
ggsave("./Figure4.png")
```
...
Conclusion there is no difference between the model with and without challenge (difference in AIC less than 2). We choose for the most parsimonious model without challenge. 
